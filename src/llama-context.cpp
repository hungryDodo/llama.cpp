...
llm_graph_result * llama_context::process_ubatch(
        ...
        int32_t layer_end,
        bool apply_memory) {
    if (mctx && apply_memory && !mctx->apply()) {
...
llm_graph_params llama_context::graph_params(
    ...
    const uint32_t resolved_layer_begin = static_cast<uint32_t>(std::max<int32_t>(0, layer_begin));
    const uint32_t resolved_layer_end = layer_end < 0
        ? model.hparams.n_layer
        : static_cast<uint32_t>(layer_end);
    return {
        /*.arch        =*/ model.arch,
        ...
        /*.n_outputs   =*/ n_outputs,
        /*.layer_begin =*/ resolved_layer_begin,
